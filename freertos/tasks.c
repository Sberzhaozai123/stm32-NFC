...
#if( configSUPPORT_DYNAMIC_ALLOCATION == 1 )//动态内存分布

	BaseType_t xTaskCreate(	TaskFunction_t pxTaskCode,//实现任务功能函数
							const char * const pcName,//任务名
							const uint16_t usStackDepth,//函数局部变量保存在任务栈 大小 malloc()
							void * const pvParameters,
	...
	taskENTER_CRITICAL();
	{
		...
		else
		{
			...
			if( xSchedulerRunning == pdFALSE )
			{
				...
				{
					pxCurrentTCB = pxNewTCB;//优先级相同 currenttask指向最后创建的任务 所有任务放到readytasklist
											//执行完最后一个任务后顺着readytasklist顺序执行 执行完的任务放到list最后
				}
				else
				{
					mtCOVERAGE_TEST_MARKER();
...
static portTASK_FUNCTION( prvIdleTask, pvParameters )
{
	...
	for( ;; )
	{
		...
		{
			extern void vApplicationIdleHook( void );//空闲任务钩子函数
